...
\textbf{QDA}. El problema es fuertemente \textbf{desbalanceado} (la grieta ocupa
$\sim$1\,\% de los p\'ixeles), por lo que el objetivo operativo es \textbf{detectar la
clase rara} sin pasarla por alto. El estudio cuantifica c\'omo cada preprocesamiento
afecta ese compromiso \emph{precision}--\emph{recall} y, en particular, la capacidad
de \textbf{detecci\'on} de la grieta (\emph{recall}).
...
Estos resultados deben leerse teniendo presente el \textbf{desbalanceo}: con la grieta
en $\sim$1\,\% de los p\'ixeles, la \emph{precision} y, con ella, el IoU/Dice se
inflan f\'acilmente \textbf{sub-segmentando} (prediciendo poca grieta se cometen pocos
falsos positivos), por lo que un valor alto en esas m\'etricas \textbf{no equivale a
una mejor detecci\'on} de la fractura. As\'i, aunque al ponderar las cuatro m\'etricas
por igual el conteo de ``mejor por celda'' (32 combinaciones) lo encabece \emph{Raw}
(""" + str(W["raw"]) + r"""/32), ese recuento est\'a dominado por
\emph{precision}/IoU/Dice y \textbf{no debe leerse como que ``Raw es el mejor''}. Las
conclusiones principales:
\begin{itemize}
  \item \textbf{Box--Cox y CLAHE mejoran la detecci\'on de la grieta.} Ambas
  \textbf{aumentan el \emph{recall} de forma significativa y consistente} en los cuatro
  conjuntos ---recuperan m\'as p\'ixeles de fractura---, que es el objetivo prioritario
  en un problema desbalanceado. El precio es una ca\'ida de \emph{precision}: el
  cl\'asico compromiso \emph{precision}--\emph{recall}. La leve ventaja de \emph{Raw}
  en IoU/Dice (que pesan ambas por igual) refleja sobre todo su \emph{precision} alta
  ``por conservadurismo'', no una mejor localizaci\'on de la grieta.
...
Sobre 200 im\'agenes con medidas de incertidumbre y tests pareados, el efecto de los
prefiltrados es un \textbf{compromiso \emph{precision}--\emph{recall}}: \textbf{Box--Cox
y CLAHE mejoran la detecci\'on de la grieta} (\emph{recall}) de forma significativa y
consistente en los cuatro conjuntos, a costa de \emph{precision}. En un problema tan
\textbf{desbalanceado}, donde el objetivo suele ser \textbf{no pasar por alto la
fractura}, esa ganancia de \emph{recall} es lo relevante, por lo que \textbf{Box--Cox o
CLAHE son preferibles}, especialmente con \textbf{QDA} (donde adem\'as IoU/Dice apenas
se degradan). La \textbf{ecualizaci\'on global de histograma (HistEq) es la excepci\'on}:
aunque tambi\'en sube el \emph{recall}, lo hace de forma err\'atica e inunda la imagen
de falsos positivos, hundiendo \emph{precision}, IoU y Dice; debe \textbf{descartarse}.
La imagen \emph{Raw} solo conviene si se invierte la prioridad hacia \textbf{maximizar
la \emph{precision}} (delimitaci\'on fina), asumiendo dejar grietas sin detectar; en ese
caso es preferible combinar Box--Cox/CLAHE con un post-procesamiento que recupere
\emph{precision} antes que renunciar a la detecci\'on.
